...
def _read(fi, position, length, dtype, left_part=True):
    ...
    if dtype == 'bcd':
        ...
        data = np.frombuffer(fi.read(int(length)), '>f8')
        return data[0] if len(data) == 1 else data
...
def _quick_merge(traces, small_number=.000001):
    ...
    assert len({tr.stats.sampling_rate for tr in traces}) == 1, "traces sampling rates are not the same"
    assert len({tr.data.dtype for tr in traces}) == 1, "traces data type are not the same"
